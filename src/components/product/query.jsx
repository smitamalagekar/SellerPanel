...
export const ProductQuestionAnswer = ({
  productName,
  initialQuestion,
  initialTimestamp,
}) => {
  ...
  const handleSend = async () => {
    if (reply.trim()) {
      try {
        const response = await axios.post("https://e-commerce-backend-1-0.onrender.com/api/productquery", {
          userName: "John Doe", // Replace with actual username
          productName: "Guitar", // Replace with actual product
          question: reply,
        });

        console.log("Sending reply:", response.data);
        setReply("");
        setSent(true);
      } catch (error) {
