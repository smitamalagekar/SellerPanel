...
const EmailTemplateCustomer = () => {
  const { customerTemplates, setCustomerTemplates, loading } = useEmailTemplates();
  ...
  const filteredTemplates = customerTemplates.filter(
    (template) =>
  ...
  const columns = [
       ...
       },
    {
      ...
      render: (template) => (
        <ActionButtons
          onEdit={() => navigate(`/marketing/email-templates/Customer/${template._id}`)}
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <PageHeader
          ...
          icon={Mail}
          onAdd={() => navigate("/marketing/email-templates/Customer/new")}
